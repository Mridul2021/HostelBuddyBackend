...
const partnerWithUs = require('../models/MessIssue');
...
router.post("/api/postMessIssue", upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    const mailOptions = {
      ...
      to: mailVIT,
      subject: "OTP for Hostel Buddy MessIssue",
...
router.post("/api/verifyMessIssueOTP", async (req, res) => {
  try {
...
router.get("/api/getMessIssue", (req, res) => {
  partnerWithUs.find()
