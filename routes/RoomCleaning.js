...
router.post("/api/postRoomCleaning", upload.single('file'), async (req, res) => {
  // Retrieve file name if uploaded
  const fileName = req.file ? req.file.originalname : null;

  // Create a new roomCleaning instance including the file name
  const newPartner = {
    ...req.body,
    filePath: fileName,
  };
...
router.post("/api/verifyRoomCleaningOTP", async (req, res) => {
  try {
    ...
    if (otp && storedOTP && otp.toString() === storedOTP.toString()) {
      ...
      const users = await roomCleaning.create(newPartner);
...
router.get("/api/getRoomCleaning", (req, res) => {
  roomCleaning.find()
